...
internal partial class PocketContainer : IEnumerable<KeyValuePair<Type, Func<PocketContainer, object?>>>
{
    ...
    private static readonly MethodInfo registerMethod =
        typeof(PocketContainer).GetMethods().Single(m => m is { Name: nameof(Register), IsGenericMethod: true });

    private static readonly MethodInfo registerSingleMethod =
        typeof(PocketContainer).GetMethods().Single(m => m is { Name: nameof(RegisterSingle), IsGenericMethod: true });

    private ConcurrentDictionary<Type, Func<PocketContainer, object?>> resolvers = new();

    private ConcurrentDictionary<Type, object> singletons = new();

    private Strategy strategyChain = _ => null;
    ...
    public object? Resolve(Type type)
    {
        ...
        {
            resolved = func?.Invoke(this) ?? null;
        }
        ...
        bool executeFirst = true)
    {
        ...
        strategyChain = !executeFirst
                            ...
                            : type => strategy(type) ?? previousStrategy(type);
        return this;
    }
    ...
    {
        Register(c => (T) singletons.GetOrAdd(typeof(T), _ => factory(c)!));
    ...
    private bool CallAfterResolve(Type type, object? resolved, [MaybeNullWhen(false)] out object replaced)
    {
        if (resolved is not null && AfterResolve?.Invoke(type, resolved) is { } o)
        {
    ...
    private static Delegate ConvertFunc(Func<PocketContainer, object> func, Type resultType)
    {
    ...
    private Func<PocketContainer, object?> ImplicitResolver<T>()
    {
        ...
        {
            if (OnFailedResolve(typeof(T), ex) is { } ex2)
            {
                ex2.Data["ResolveFailed"] = true;
                throw ex2;
            }

            defaultFactory = _ => default;
        }
    ...
    private static MethodCallExpression CallResolve(ParameterExpression container, Type type) =>
        Expression.Call(container, resolveMethod.MakeGenericMethod(type));

    private static MethodCallExpression CallResolveOptional(ParameterExpression container, Type type) =>
